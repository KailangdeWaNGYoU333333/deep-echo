...
def parse_s_expr(s: str):
    """从字符串解析 S-表达式为 GeneNode"""
    s = s.strip()
    if not s or not s.startswith('('):
        if s in VARS or s in NUMS:
            return GeneNode(s, [])
        # 尝试作为数字或变量返回
        return GeneNode(s, [])
    ...
    try:
        ...
        # 可能只有一个操作符没有参数
        return GeneNode(s[1:-1].strip(), [])
    
    # 提取参数部分
    args_str = s[op_end+1:-1].strip()
    if not args_str:
        return GeneNode(op, [])
    ...
    for c in args_str:
        if c == '(':
            ...
            current += c
            if depth == 0:
                parsed = parse_s_expr(current.strip())
                if parsed:
                    args.append(parsed)
                current = ""
        elif c == ' ' and depth == 0:
            if current.strip():
                args.append(parse_s_expr(current.strip()))
                current = ""
        else:
            current += c
    if current.strip():
    ...
    return GeneNode(op, [a for a in args if a is not None])