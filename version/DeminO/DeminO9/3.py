...
from gene_syntax import GeneNode, random_gene, mutate_gene, crossover_genes, parse_s_expr
            ...
            if not success:
                for line in result.stderr.split("\n"):
                    ...
                    if "error" in line_lower:
                        if "unknown identifier" in line_lower: errors.append({"type": "unknown_identifier"})
                        elif "type mismatch" in line_lower: errors.append({"type": "type_mismatch"})
                        elif "unsolved goals" in line_lower: errors.append({"type": "unsolved_goals"})
...
class GeneTreeEvolver:
    """在语法树上进化的证明搜索器（含突变顾问）"""

    def __init__(self, verifier, pop_size=200, max_depth=5):
        self.verifier = verifier
        self.pop_size = pop_size
        self.max_depth = max_depth
        self.population: List[GeneNode] = []
        self.best: Optional[tuple] = None
        self.generation = 0
        self.current_target = "1 + 0 = 1"
        self.use_advisor = True
        self.advisor = None  # 延迟加载，避免启动时加载两次模型

    def _load_advisor(self):
        if self.advisor is None and self.use_advisor:
            try:
                from mutation_advisor import MutationAdvisor
    ...
    def initialize(self):
        self.population = [random_gene(self.max_depth) for _ in range(self.pop_size)]
        self.best = None
        self.generation = 0

    def _fitness(self, verification, gene):
        if verification.get("success", False):
            return 200.0 + max(0, 30 - len(repr(gene)) / 10)
        score = 40 - verification.get("error_count", 1) * 6
        progress = self.verifier.estimate_progress(gene.to_lean(self.current_target))
        score += progress * 30
        return max(0, score)

    def evolve_generation(self):
        print(f"\n🧬 第 {self.generation} 代进化中（语法树模式）...")

        # 并发评估
        tasks = [(g, self.verifier, self.current_target, self._fitness) for g in self.population]
        candidates = []
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as pool:
            for i, result in enumerate(pool.map(_eval_tree, tasks, chunksize=20)):
        ...
        while len(new_pop) < self.pop_size:
            ...
            child = mutate_gene(child, rate=0.3, depth=self.max_depth)
        ...
        if self.generation % 3 == 0:
            ...
            for i in range(reset_count):
                new_pop[-(i+1)] = random_gene(self.max_depth)

        # 突变顾问：每5代对最佳个体提问
        if self.generation % 5 == 0 and self.generation > 0 and self.best:
            if self.use_advisor:
                self._load_advisor()
                if self.advisor:
                    print(f"  🤖 调用突变顾问...")
                    gene_repr = repr(self.best[0])
                    suggestions = self.advisor.suggest_mutations(gene_repr, self.current_target, n=3)
                    for gene_str, reason in suggestions:
                        try:
                            new_gene = parse_s_expr(gene_str)
                            if new_gene and len(new_pop) < self.pop_size + 5:
                                new_pop.append(new_gene)
                                print(f"    ✅ 采纳建议: {reason[:60]}")
                        except:
                            pass
        ...
        print(f"  最佳: {candidates[0][1]:.1f} | 平均: {avg:.1f} | 基因: {repr(candidates[0][0])[:60]}")
    ...
    def run(self, theorem=None, max_generations=300):
        ...
        if theorem:
            ...
            self.current_target = "1 + 0 = 1"
        print(f"🚀 开始语法树进化搜索\n   种群: {self.pop_size} | 最大代数: {max_generations} | 目标: {self.current_target}")

        for gen in range(max_generations):
            if self.evolve_generation():
                print(f"\n✨ 第 {gen} 代进化成功！")
                break
            if gen % 20 == 0 and self.best:
                print(f"\n📜 最佳基因: {repr(self.best[0])[:80]}")

        return self.best

def _eval_tree(args):
    gene, verifier, target, fitness_fn = args
    lean_code = gene.to_lean(target)
