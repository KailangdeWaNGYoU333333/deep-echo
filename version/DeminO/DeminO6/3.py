...
class TheoremStringEvolver:
    ...
    def decode_to_lean(self, gene):
        lines = ["example : 1 + 0 = 1 := by"]
        for token in gene:
    ...
    def _fitness(self, verification, gene):
        ...
        progress = self.verifier.estimate_progress(self.decode_to_lean(gene))
        score += progress * 30
        return max(0, score)
    ...
    def evolve_generation(self):
        ...
        # 并发验证
        tasks = [(g, self.verifier, self.decode_to_lean, self._fitness) for g in self.population]
        candidates = []
...
def _eval_one(args):
    gene, verifier, decode_fn, fitness_fn = args
    lean_code = decode_fn(gene)
