...
class TheoremStringEvolver:
    ...
    def decode_to_lean(self, gene):
        """把基因序列翻译回 Lean 代码"""
        reverse_theorem = {v: k for k, v in self.theorem_map.items()}
        reverse_rule = {v: k for k, v in self.rule_map.items()}

        lines = ["example : 1 + 0 = 1 := by"]
        for token in gene:
            if token in reverse_theorem:
                lines.append(f"  have h := by ring")
            elif token in reverse_rule:
                lines.append(f"  {reverse_rule[token]}")
            elif token == "R003":
                lines.append(f"  calc")
                lines.append(f"    1 + 0 = 1 := by ring")
                lines.append(f"    _ = 1 := rfl")
    ...
    def evolve_generation(self):
        ...
        for i, gene in enumerate(self.population):
            lean_code = self.decode_to_lean(gene)
            verif = self.verifier.verify(lean_code)
            fit = self._fitness(verif, gene)
            candidates.append((gene, fit, verif))
            if (i + 1) % 100 == 0:
